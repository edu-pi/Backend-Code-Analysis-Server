...
from app.visualize.analysis.stmt_parser.expr_analysis.expr_parser.binop_parser import BinopParser


@pytest.mark.parametrize("left_obj, right_obj, op, expected", [
    (
...
def test_parse(left_obj, right_obj, op, expected):
    result = BinopParser.parse(left_obj, right_obj, op)
    assert result == expected


@pytest.mark.parametrize("left_value, right_value, op, expected", [
    (1, 2, ast.Add(), 3),
    (3, 1, ast.Sub(), 2),
    (3, 2, ast.Mult(), 6),
    (3, 2, ast.Div(), 1.5),
...
def test_calculate_value(left_value, right_value, op, expected):
    result = BinopParser._calculate_value(left_value, right_value, op)
    assert result == expected


@pytest.mark.parametrize("left_obj, right_obj, op, value, expected", [
    (["1"], ["2"], ast.Add(), 3, ["1 + 2", "3"]),
    (["a", "3"], ["1"], ast.Add(), 4, ["a + 1", "3 + 1", "4"]),
    (["a", "3"], ["b + 1", "2 + 1", "3"], ast.Add(), 6, ["a + b + 1", "3 + 2 + 1", "3 + 3", "6"]),
    (["a + 1", "3 + 1", "4"], ["b + 2", "2 + 2", "4"], ast.Add(), 8, ["a + 1 + b + 2", "3 + 1 + 2 + 2", "4 + 4", "8"]),
    (["a + 1", "3 + 1", "4"], ["b + 2", "2 + 2", "4"], ast.Mult(), 8,
     ["a + 1 * b + 2", "3 + 1 * 2 + 2", "4 * 4", "8"])
])
def test_creat_expressions(left_obj, right_obj, op, value, expected):
    result = BinopParser._create_expressions(left_obj, right_obj, op, value)
    assert result == expected


@pytest.mark.parametrize("left_expression, right_expression, op, expected", [
    ("a", "b", ast.Add(), "a + b"),
    ("a", "b", ast.Sub(), "a - b"),
    ("a", "b", ast.Mult(), "a * b"),
    ("a", "b", ast.Div(), "a / b"),
...
def test_concat_expression(left_expression, right_expression, op, expected):
    result = BinopParser._concat_expression(left_expression, right_expression, op)
    assert result == expected
