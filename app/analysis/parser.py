...
def assign_parse(node):
    for target in node.targets:
        depth = CodeElementManager.depth

        # BinOp인 경우
        if isinstance(node.value, ast.BinOp):
            parsed_expressions = binOp_parse(node.value)
            for parsed_expression in parsed_expressions:
                CodeElementManager.add_step(
                    Variable(depth=depth, name=target.id, expr=parsed_expression)
                )
            CodeElementManager.add_variable_value(name=target.id, value=parsed_expressions[-1])

        # 상수인 경우
        elif isinstance(node.value, ast.Constant):
            value = constant_parse(node.value)
            CodeElementManager.add_variable_value(name=target.id, value=value)
            CodeElementManager.add_step(
                Variable(depth=depth, name=target.id, expr=CodeElementManager.get_variable_value(name=target.id))
            )
...
def __create_intermediate_expression(expr, result):
    ...
    for var in variables:
        value = CodeElementManager.get_variable_value(var)
...
def name_parse(node):
    try:
        return CodeElementManager.get_variable_value(name=node.id)
...
def for_parse(node):
    ...
    for_id = CodeElementManager.get_call_id(node)
    ...
    for i in range(condition.start, condition.end, condition.step):
        # target 업데이트
        CodeElementManager.add_variable_value(name=target_name, value=i)
        CodeElementManager.add_step(
            For(id=for_id, depth=CodeElementManager.get_depth(), condition=condition)
        )
        CodeElementManager.increase_depth()
        ...
        for child_node in node.body:
            if isinstance(child_node, ast.Expr):
                ...
                for parsed_obj in parsed_objs:
                    CodeElementManager.add_step(parsed_obj)

            elif isinstance(child_node, ast.For):
                for_parse(child_node)
        CodeElementManager.decrease_depth()
...
def print_parse(node: ast.Call):
    ...
    for cur_node in node.args:
        if isinstance(cur_node, ast.BinOp):
            ...
            # 중간 연산 과정이 포함된 노드 생성
            for parsed_expression in parsed_expressions:
                print_obj = Print(id=CodeElementManager.get_call_id(node), depth=CodeElementManager.get_depth(), name='print', expr=parsed_expression)
